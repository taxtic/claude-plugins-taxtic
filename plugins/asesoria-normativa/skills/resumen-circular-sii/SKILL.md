...
4. **El título y la bajada del documento tampoco los escribes tú.** Se ensamblan desde `fuente.json`.
...
7. **Una sección obligatoria sin sustento no se rellena.** Va como ítem `derivada` de `a_verificar` con `suple_seccion` apuntando a la sección que suple. Solo se suple una sección obligatoria del perfil, ausente y no suplida ya por otro ítem. `a_verificar` no se suple a sí misma.

```json
{ "texto": "Confirmar el procedimiento aplicable con la unidad respectiva.", "suple_seccion": "procedimiento" }
```
...
Cualquier otra cifra **falla cerrada**: `48 horas`, `el N° 4 del artículo 97`, `el día 30`, `agosto de 2026` sin día o `el 5° día hábil` emiten un centinela que ninguna cita puede respaldar, y el gate rechaza esa afirmación. No es un error de la cita: es la forma de escribir el dato. Cuando pase, reescribe la cifra en una de las formas de arriba, o sácala de tu texto y deja que la cita la lleve. En una `derivada` esto es absoluto: no puede aparecer ningún dígito.
...
El gate rechaza con código 1 e imprime `RECHAZADO [<clase>] <ruta del bloque>: <motivo>`. Corrige **ese bloque** y vuelve a correr. No sigas al builder con el gate en rojo.

`exportar_pdf.py` nunca falla duro: si Word no está disponible o la conversión se cae, imprime un aviso y el `.docx` sigue siendo la entrega.
