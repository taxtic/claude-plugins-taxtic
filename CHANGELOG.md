...
### Changed
- Plugin `asesoria-normativa` v0.2.0: la skill `/resumen-circular-sii` pasa de un resumen en markdown con plantilla fija a un documento Word y PDF con marca TAXTIC, con estructura elegida según el tipo de documento (circular, resolución exenta u oficio) y un gate que exige procedencia verificable para todo el texto emitido: ensamblado desde el documento, tomado de un catálogo cerrado, redactado con cita textual obligatoria, criterio profesional confinado a dos secciones y sin datos verificables, o dato aportado por el usuario (identidad del documento y nombre de quien firma). El tipo y el número se detectan solo del recuadro identificatorio del SII y, sin él, la extracción se detiene sin escribir `fuente.json` en vez de identificar el documento por una norma citada en el cuerpo. Agrega al plugin `scripts/`, `tests/`, los assets de marca y `requirements.txt`.
